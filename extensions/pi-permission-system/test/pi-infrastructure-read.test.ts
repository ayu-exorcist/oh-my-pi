...
describe("discoverGlobalNodeModulesRoot", () => {
  ...
  test("the discovered path includes the pi-permission-system package directory", () => {
    ...
    expect(posix.join(root!, "pi-permission-system")).toBe(
...
const INFRA_DIRS = [
  "/home/user/.pi/agent",
  "/home/user/.pi/agent/ayu",
  "/home/user/.pi/agent/ayu/git",
...
describe("isPiInfrastructureRead", () => {
  ...
  test("allows 'read' tool for a file inside agentDir", () => {
    expect(
      isPiInfrastructureRead(
        "read",
        "/home/user/.pi/agent/ayu/extensions/pi-permission-system/config.json",
        INFRA_DIRS,
        CWD,
      ),
    ).toBe(true);
  ...
  test("allows 'grep' tool for a path inside agentDir/git", () => {
    expect(
      isPiInfrastructureRead(
        "grep",
        "/home/user/.pi/agent/ayu/git/some-package/README.md",
        INFRA_DIRS,
        CWD,
      ),
    ).toBe(true);
  ...
  test("does not allow 'read' for a path only partially matching an infra dir prefix", () => {
    ...
    expect(
      isPiInfrastructureRead("read", "/home/user/.pi/agent-other/config.json", INFRA_DIRS, CWD),
    ).toBe(false);
  });
  ...
  test("returns false when infrastructureDirs is empty but path IS project-local .pi/npm", () => {
    ...
    expect(isPiInfrastructureRead("read", `${CWD}/.pi/npm/node_modules/x/SKILL.md`, [], CWD)).toBe(
      true,
    );
  });
});
...
describe("isPiInfrastructureRead with glob patterns", () => {
  ...
  test("? matches exactly one character", () => {
    expect(
      isPiInfrastructureRead("read", "/opt/homebrew/X/file.md", ["/opt/homebrew/?/file.md"], CWD),
    ).toBe(true);
  });

  test("? does not match multiple characters", () => {
    expect(
      isPiInfrastructureRead("read", "/opt/homebrew/abc/file.md", ["/opt/homebrew/?/file.md"], CWD),
    ).toBe(false);
  });
  ...
  test("mixed array of plain dirs and glob patterns — both branches work", () => {
    ...
    expect(isPiInfrastructureRead("read", "/home/user/.pi/agent/config.json", dirs, CWD)).toBe(
      true,
    );
    expect(
      isPiInfrastructureRead(
  ...
  test("plain entry with ~ prefix matches after home expansion", () => {
    const home = homedir();
    expect(
      isPiInfrastructureRead("read", `${home}/.pi/agent/config.json`, ["~/.pi/agent"], CWD),
    ).toBe(true);
  });
