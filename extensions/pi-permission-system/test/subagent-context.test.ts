...
import { isSubagentExecutionContext, normalizeFilesystemPath } from "#src/subagent-context";
...
describe("isSubagentExecutionContext — registry detection", () => {
  ...
  test("returns false when registry is provided but session dir is not registered", () => {
    const registry = new SubagentSessionRegistry();
    expect(isSubagentExecutionContext(makeCtx(outsideDir), subagentRoot, registry)).toBe(false);
  });
