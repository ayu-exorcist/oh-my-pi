...
import { getEventInput, PermissionGateHandler } from "#src/handlers/permission-gate-handler";
...
function makeToolCallEvent(toolName: string, extraFields: Record<string, unknown> = {}) {
  return {
    type: "tool_call",
    toolCallId: "tc-1",
    name: toolName,
...
function makePermissionResult(state: "allow" | "deny" | "ask"): PermissionCheckResult {
...
function makeSession(
  ...
  return {
    ...
    getInfrastructureDirs: vi.fn().mockReturnValue(["/test/agent", "/test/agent/git"]),
...
describe("handleToolCall — path gate (tools)", () => {
  it("blocks a read of .env when path surface denies *.env", async () => {
    const checkPermission = vi
      .fn()
      .mockImplementation((surface: string, _input: unknown, _agentName?: string) => {
        if (surface === "path") {
