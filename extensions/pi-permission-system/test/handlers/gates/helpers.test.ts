...
import { deriveDecisionValue, deriveResolution } from "#src/handlers/gates/helpers";
...
describe("deriveResolution", () => {
  it("returns policy_allow for allow state", () => {
    expect(deriveResolution("allow", "allow", false, true)).toBe("policy_allow");
  });
  ...
  it("returns user_approved_for_session for ask + allow with session", () => {
    expect(deriveResolution("ask", "allow", true, true)).toBe("user_approved_for_session");
  });

  it("returns auto_approved when autoApproved flag is set", () => {
    expect(deriveResolution("ask", "allow", false, true, true)).toBe("auto_approved");
  });
  ...
  it("returns confirmation_unavailable for ask + block without canConfirm", () => {
    expect(deriveResolution("ask", "block", false, false)).toBe("confirmation_unavailable");
  });
});
