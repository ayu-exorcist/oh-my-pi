...
describe("formatDenyReason", () => {
  describe("tool context", () => {
    ...
    test("MCP source with target on non-mcp toolName", () => {
      expect(
        formatDenyReason(toolCtx(toolCheck("anything", { source: "mcp", target: "server:tool" }))),
      ).toBe("[pi-permission-system] is not permitted to run MCP target 'server:tool'.");
    });
  });
...
describe("formatUserDeniedReason", () => {
  ...
  describe("path context", () => {
    ...
    test("with reason", () => {
      ...
      ).toBe("[pi-permission-system] User denied access to path '/etc/passwd'. Reason: sensitive.");
    });
  });
