...
import { getAyuAgentDir, getGlobalConfigPath, joinPathLike } from "./config-paths";
...
function defaultAgentsDir(): string {
  return joinPathLike(getAyuAgentDir(getAgentDir()), "agents");
}
function defaultGlobalMcpConfigPath(): string {
  return joinPathLike(getAyuAgentDir(getAgentDir()), "mcp.json");
}
...
export class FilePolicyLoader implements PolicyLoader {
  ...
  private loadScopeConfigFrom(
    ...
    const filePath = joinPathLike(dir, `${agentName}.md`);
  ...
  getCacheStamp(agentName?: string): string {
    const agentStamp = agentName
      ? getFileStamp(joinPathLike(this.agentsDir, `${agentName}.md`))
      : "missing";
    ...
    const projectAgentStamp =
      this.projectAgentsDir && agentName
        ? getFileStamp(joinPathLike(this.projectAgentsDir, `${agentName}.md`))
        : "none";
