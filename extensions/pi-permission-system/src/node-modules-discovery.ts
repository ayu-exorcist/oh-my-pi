 ...
 */
function filePathFromUrl(fromUrl: string): string | null {
  try {
...
function walkUpToNodeModules(fromUrl: string): string | null {
  const thisFile = filePathFromUrl(fromUrl);
  if (!thisFile) {
    return null;
  }

  const usesPosixStyle = thisFile.startsWith("/") && !/^[A-Za-z]:/.test(thisFile);
  const dirnameForPath = usesPosixStyle ? posix.dirname : dirname;
  const basenameForPath = usesPosixStyle ? posix.basename : basename;
  let dir = dirnameForPath(thisFile);
  while (dir !== dirnameForPath(dir)) {
    if (basenameForPath(dir) === "node_modules") {
      return dir;
    }
    dir = dirnameForPath(dir);
  }
  return null;
}
