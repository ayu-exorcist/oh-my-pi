...
import type { PermissionPromptDecision, RequestPermissionOptions } from "#src/permission-dialog";
import {
...
export async function waitForForwardedPermissionApproval(
  ...
  const targetSessionId = resolvePermissionForwardingTargetSessionId({
    hasUI: ctx.hasUI,
    isSubagent: isSubagentExecutionContext(ctx, deps.subagentSessionsDir, deps.registry),
...
export async function processForwardedPermissionRequests(
  ...
  const location = getExistingPermissionForwardingLocation(deps.forwardingDir, currentSessionId);
  if (!location) {
    return;
  }
...
export async function confirmPermission(
  ...
  if (!isSubagentExecutionContext(ctx, deps.subagentSessionsDir, deps.registry)) {
    return { approved: false, state: "denied" };
  }
