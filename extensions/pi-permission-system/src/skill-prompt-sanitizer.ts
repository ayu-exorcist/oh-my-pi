import { isPathWithinDirectory, normalizePathForComparison } from "./path-utils";
...
function createResolvedSkillEntry(
  ...
  const normalizedLocation = normalizePathForComparison(entry.location, cwd);
  return {
    name: entry.name,
    ...
    state,
    normalizedLocation,
    normalizedBaseDir: getParentPath(normalizedLocation),
  };
}
...
export function resolveSkillPromptEntries(
  ...
  for (const section of sections) {
    ...
    replacements.push({
      ...
      content:
        visibleSectionEntries.length > 0 ? renderAvailableSkillsSection(visibleSectionEntries) : "",
    });
  }
