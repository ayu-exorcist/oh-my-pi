...
export function describeExternalDirectoryGate(
  ...
  const normalizedExtPath = normalizePathForComparison(externalDirectoryPath, tcc.cwd);

  // ── Pi infrastructure read bypass ──────────────────────────────────────
  if (isPiInfrastructureRead(tcc.toolName, normalizedExtPath, infraDirs, tcc.cwd)) {
    return {
      action: "allow",
      log: {
