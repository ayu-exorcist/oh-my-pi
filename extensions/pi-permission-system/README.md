...
This package is vendored into `@ayulab/oh-my-pi` from `@gotgenes/pi-permission-system@7.4.1` and adapted for Ayu config paths.
...
This extension is bundled as a local workspace package in `@ayulab/oh-my-pi`; no separate npm install is required when using the oh-my-pi package.

## Quick Start

1. Create the global config file at `~/.pi/agent/ayu/extensions/pi-permission-system/config.json`:
...
| Scope   | Path                                                          |
| ------- | ------------------------------------------------------------- |
| Global  | `~/.pi/agent/ayu/extensions/pi-permission-system/config.json` |
| Project | `<cwd>/.pi/ayu/extensions/pi-permission-system/config.json`   |
