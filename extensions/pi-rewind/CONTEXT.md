...
- **Resume file restore includes startup-selected sessions**: Pi's `pi -r` picker starts the chosen session through the startup lifecycle rather than the in-process `/resume` switch lifecycle. `ayu.checkpoint.restoreOnResume` is intentionally the single user-facing switch for opening existing sessions, so it applies to both `session_start` reason `"resume"` and `"startup"` when a target Checkpoint exists. If an existing session has checkpointed file changes but its storage is missing, show a warning-themed widget above the editor input without creating blank storage; clear that widget on the next user input.
- **Tree file restore defaults to ask**: Pi-native `/tree` conversation navigation is preserved, and `pi-rewind` defaults `ayu.rewind.restoreOnTree` to `"ask"`. After Pi's native `No summary` choice, ask mode offers file sync only when the session has ever produced checkpointed file changes; one changed checkpoint keeps later `/tree` prompts enabled for that session. Set `restoreOnTree` to `"never"` to keep `/tree` conversation-only. In `always`, file sync force restores checkpoint-managed files to the target checkpoint without a dirty-workspace prompt. Summary navigation does not restore files.
- **`Sync files?` Esc cannot return to `/tree` yet**: In `restoreOnTree: "ask"`, Pi opens the extension `Sync files?` dialog only after the built-in tree selector has already called `done()` and closed. Without changing Pi's package implementation, an extension cannot keep `/tree` open or return focus to its previous level, so Esc cancels the sync prompt instead of navigating back inside `/tree`. Pressing Esc is equivalent to selecting `No` — the file sync is skipped and tree navigation proceeds with conversation-only state change. If Pi exposes a supported hook before the selector closes, implement the behavior through that API rather than emulating tree navigation.
...
- **Session deletion is not inferred**: If users delete sessions via `pi -r` or `/resume` with `Ctrl+D`, Pi currently has no documented deletion lifecycle hook for extensions. Do not auto-delete checkpoint storage by guessing from missing JSONL files. `/checkpoint` deletion protects only the current Pi instance's active session storage; it does not implement cross-instance busy detection for another Pi process using the same storage. If Pi later adds `session_before_delete` / `session_deleted`, use that exact hook to delete only the matching session's checkpoint storage and clear related in-memory state.
