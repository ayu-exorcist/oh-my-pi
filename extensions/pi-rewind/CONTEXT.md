...
- **Tree file restore defaults to ask**: Pi-native `/tree` conversation navigation is preserved, and `pi-rewind` defaults `ayu.rewind.restoreOnTree` to `"ask"`. After Pi's native `No summary` choice, ask mode offers file sync only when the current-to-target tree navigation range includes checkpointed file changes. Set `restoreOnTree` to `"never"` to keep `/tree` conversation-only. In `always`, file sync force restores checkpoint-managed files to the target checkpoint without a dirty-workspace prompt when that navigation range includes file changes. Summary navigation does not restore files.
