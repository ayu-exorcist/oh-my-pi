...
- **Changed-path capture is a possible future architecture**: A Turn-level changed-path log with before/after content capture could reduce full-tree staging and support richer multi-root behavior, but it requires reliable write detection for arbitrary shell commands plus restore semantics for deletes, renames, binary files, permissions, dirty guards, and rollback. Until then, git-backed Checkpoint Storage remains the source of truth.
