...
/** Execute `fn` only if the repo lock can be acquired immediately. */
export async function tryWithRepoLock<T>(
  repoDir: string,
  fn: () => Promise<T>,
): Promise<{ readonly locked: true; readonly value: T } | { readonly locked: false }> {
  const lockPath = path.join(repoDir, LOCK_DIR_NAME);
  const locked = await tryAcquire(lockPath);
