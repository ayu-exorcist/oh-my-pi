...
Create a Changesets entry for user-facing package changes before merging:

```bash
pnpm run changeset
```

For infrastructure-only changes that should not publish packages, create an empty changeset:

```bash
pnpm changeset add --empty
```

Changesets creates and updates the version PR in CI. For manual versioning, apply pending version bumps and update the lockfile before publishing:

```bash
pnpm run version-packages
```

`pnpm run release` builds, validates, and delegates npm publishing plus git tag creation to `changeset publish`.
